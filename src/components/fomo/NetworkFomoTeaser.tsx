      ...
      <div className="pointer-events-none grid grid-cols-1 gap-3 blur-sm sm:grid-cols-2">
        {GHOST_ROWS.map((w, i) => (
          <div key={i} className="flex items-center gap-3 rounded-lg border border-border bg-card p-4">
            <span className="h-10 w-10 shrink-0 rounded-full bg-card-elevated" />
            <div className="flex flex-1 flex-col gap-2">
              <span className={`h-3.5 ${w} rounded bg-card-elevated`} />
              <span className="h-3 w-24 rounded bg-card-elevated" />
            </div>
          </div>
        ))}
      </div>
