  ...
  return (
    <div className="relative">
      <div className="pointer-events-none grid grid-cols-1 gap-4 blur-sm sm:grid-cols-2 lg:grid-cols-3">
        {GHOST_CARDS.map((c, i) => (
          <div key={i} className="flex flex-col gap-3 border border-border bg-card p-5">
            <div className="flex items-center justify-between">
              <span className={`h-4 ${c.badge} rounded-full bg-gold/20`} />
            </div>
            <span className={`h-4 ${c.title} bg-card-elevated`} />
            {c.lines.map((w, j) => (
              <span key={j} className={`h-3 ${w} bg-card-elevated`} />
            ))}
            <span className="mt-2 h-8 w-full bg-card-elevated" />
          </div>
        ))}
      </div>
